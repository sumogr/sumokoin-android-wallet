...
class ExchangeRateImpl implements ExchangeRate {
    ...
    ExchangeRateImpl(final JSONObject jsonObject, final boolean sumokoinAssets) throws JSONException, ExchangeException {
        try {
            ...
            baseCurrency = sumokoinAssets ? quoteC : baseC;
            quoteCurrency = sumokoinAssets ? baseC : quoteC;
            JSONObject quote = quotes.getJSONObject(key);
            double price = quote.getDouble("price");
            this.rate = sumokoinAssets ? (1d / price) : price;
